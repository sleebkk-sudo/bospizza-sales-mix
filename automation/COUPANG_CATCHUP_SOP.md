...
## 11. 옵션(사이즈/도우/토핑 등) 캡처 — 2026-09-03 추가

기존 스크립트는 상품명/수량/금액만 잡고 옵션 줄은 아예 건너뛰고 있었음(사용자가
스크린샷으로 지적: "볼 수 있는데 왜 안봐?"). 실제로는 DOM에 옵션이 다 있고, 대시보드도
이미 `menu_option_selections` 테이블을 읽는 옵션 믹스 UI가 붙어있었음 — 그냥 스크래퍼가
안 채우고 있었던 것.

**DOM 구조**: `.order-detail-item .col-7 > UL.item-options > LI.row > SPAN.order-detail-option-name`
가 선택된 옵션 각각을 텍스트로 담고 있음(예: "M(6조각)", "고구마링", "콜라 1.25L (리뷰용)",
"피클", "우유도우").

**분류 로직**은 배민/요기요 파서(`src/lib/parseSalesReport.ts`의 `classifyOptionSelection`)와
동일한 카테고리 체계를 그대로 씀(사이즈/도우 선택/추가토핑/추가메뉴/음료 추가/리뷰이벤트/
사이드 선택). 쿠팡만 사이즈가 "M(6조각)"/"L(8조각)" 형태라 `/^([ML])\(\d+조각\)$/` 정규식으로
"M"/"L"로 정규화하는 보정 한 줄만 추가.

**쿠팡의 한계**: 옵션별 개별 가격이 주문 화면에 안 나오고 전체 주문 금액만 나옴 —
그래서 쿠팡 옵션 행은 항상 `revenue: 0`으로 넣는다(건수만 정확, 매출은 배민/요기요만
정확). 대시보드의 "옵션별 판매 믹스" 표는 이후 다른 세션에서 실매출 대신 고정 옵션가
카탈로그(`src/lib/optionPriceCatalog.ts`) 기준으로 가격/매출을 계산하도록 바뀌었으니,
쿠팡 revenue=0이어도 화면엔 카탈로그 단가 × 건수로 정상 표시됨 — 이상하다고 다시 고치려
하지 말 것.

메인 캐치업 스크립트(매출+옵션 한 번에 upsert)에 옵션 스텝을 표준으로 포함시켰음 —
이제 매장 하나 처리할 때 `menu_sales_items` + `store_daily_stats` + `menu_option_selections`
세 테이블을 한 번의 `run()` 호출로 같이 넣는다.

## 12. 데일리 패스트 트랙 — 2026-09-03 확립 (날짜피커 클릭 생략)

매일(또는 거의 매일) 반영할 때는 6절의 "1개월 프리셋 클릭" 자체를 생략해도 된다.
매장 페이지 기본 진입 시 뜨는 날짜 범위가 이미 "최근 7일(오늘 포함)"이라서, 전날치
신규 주문은 스크롤/클릭 없이 첫 로드 화면에 다 잡힌다. 검증(2026-09-03, 48개 매장
전수, ACCESS_DENIED 0건): `navigate` → 2초 대기 → 곧바로 주문 목록 파싱, 날짜 필터
클릭(`.css-1uvczjr` 열기 → "1개월" 클릭 → 검색 버튼 클릭, 약 2.9초 소요)을 완전히
스킵. 매장당 스텝이 절반으로 줄고 요청 자체도 줄어서 ACCESS_DENIED 위험이 낮아진다.

```js
// 데일리용 최소 스크립트 뼈대 (날짜피커 조작 없음)
async function run(anonKey, storeId, minDate) { // minDate = "어제" ISO, 예: "2026-09-02"
  if (document.body.textContent.includes('권한이 존재하지 않습니다')) return {error:'ACCESS_DENIED', storeId};
  // ... 상품명/옵션 추출은 기존과 동일 ...
  const allOrders = extractOrders(); // 필터 없이 현재 화면에 보이는 주문 전부
  const orders = allOrders.filter(o => toISO(o.date) >= minDate); // 여기서 어제 이후만 골라냄
  // ... 이하 upsert는 동일 ...
}
```

캐치업(며칠~몇 주 밀린 경우)에는 여전히 6절의 1개월 프리셋이 필요함 — 데일리 패스트
트랙은 "마지막 반영일이 어제 또는 그제"인 경우에만 쓸 것. 그보다 오래 밀렸으면 먼저
1개월(또는 3개월) 프리셋으로 캐치업 한 번 하고, 그다음부터 데일리 트랙으로 전환.

## 13. 매장명 이중 등록 버그 — 갈산점 (2026-09-03 발견 및 수정)

매장 ID 1057915의 실제 화면 표기가 "보스피자 부평갈산점"이라, 표준 정규화 로직을
그대로 적용하면 `보스피자-부평갈산점`이라는 **새 이름**이 만들어진다. 그런데 이 매장은
원래 `보스피자-갈산점`이라는 이름으로 데이터가 쌓여 있었음(다른 소스/세션에서 이 이름을
써왔던 것으로 보임) — 그 결과 같은 매장이 두 이름으로 쪼개져서 매출/옵션이 반씩
나뉘어 잡히는 데이터 오류가 있었다.

**고침**: 1) 기존 `보스피자-부평갈산점` 밑에 있던 `menu_sales_items`/`store_daily_stats`/
`menu_option_selections`/`stores` 행을 전부 `보스피자-갈산점`으로 SQL로 합치고 삭제.
2) 이 매장(ID 1057915) 전용으로 스크립트에 `if (storeName === "보스피자-부평갈산점")
storeName = "보스피자-갈산점";` 보정 한 줄을 넣어서, 앞으로도 계속 정규화 결과를
`갈산점`으로 강제한다. 이 ID를 다시 처리할 때 이 보정 줄을 빼먹지 말 것 — 빼먹으면
또 갈라진다.
